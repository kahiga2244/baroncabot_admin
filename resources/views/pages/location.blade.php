...
<body>
   {{-- <nav class="navbar navbar-expand-lg navbar-dark ftco_navbar ftco-navbar fixed-top" style="background-color: #002748;" id="ftco-navbar">
       ...
       <div  class="order-sm-start order-lg-last">
        <a href="tel:+442036951739">
        <button  class="form-control btn btn-outline-white"><span class="fa fa-phone">+44 20 3695 1739</span></button>
        </a>
      </div>
      ...
      <div class="collapse navbar-collapse" id="ftco-nav">
        <ul class="navbar-nav m-auto">
           <li class="nav-item active"><a href="#" class="nav-link">Projects</a></li>
...
<!-- footer end -->
  ...
  {{-- <script>
   ...
   function captureSignature() {
     const signatureData = canvas.toDataURL(); // Convert canvas content to a data URL
     signatureDataInput.value = signatureData; // Assign the signature data to the hidden input field
   }

   // Add an event listener to capture the signature when the form is submitted
...
{{-- <script>
    ...
    document.querySelector('form').addEventListener('submit', captureSignature);
    console.log(signatureDataInput.value)
</script> --}}
{{-- <script>
const canvas = document.getElementById('signatureCanvas');
const signaturePad = new SignaturePad(canvas);
...
function captureSignature() {
    ...
    // Convert the signature data to a Blob object
    const signatureData = signaturePad.toDataURL();
    const signatureBlob = dataURLtoBlob(signatureData);

    // Create a FormData object and append the signature Blob
    const formData = new FormData();
    formData.append('signature', signatureBlob, 'signature.png');

    // Send the FormData object to your server using AJAX or submit the form
    // Example using AJAX:
    fetch('/proof-form', {
        method: 'POST',
        body: formData
    ...
    .then(response => {
        ...
        // e.g., show success message or redirect to another page
    })
    .catch(error => {
...
{{-- <script>
   ...
   function captureSignature() {
       ...
       const signatureData = signaturePad.toDataURL();
       signatureDataInput.value = signatureData;
       // Remove the "data:image/png;base64," prefix to get only the base64-encoded image data
       const base64Data = signatureData.replace(/^data:image\/(png|jpeg|jpg);base64,/, '');

       // Assign the base64 data to the hidden input field
